...
- `404 Not Found` — usually a typo in the package name or a stale npm cache;
...
The dashboard uses the same TypeScript client SDK (`@tylerwarburton/sprite-core-client`)
...
```sh
# from repo root
pnpm --filter @tylerwarburton/sprite-core-ui build       # one-shot build
pnpm --filter @tylerwarburton/sprite-core-ui dev         # Vite dev server (HMR)
```
