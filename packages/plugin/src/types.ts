...
/**
 * Per-agent link to a pixellab.ai character. Set when the operator links the
 * agent through the dashboard (or the exporter's `--apply` step) so future
 * dashboard actions — animate / regenerate / re-export — know which
 * upstream character to talk to.
 */
export type SpriteCorePixellabLink = {
  /** Pixellab character UUID. */
  characterId: string;
  /** Last time we successfully ran an export against this character (epoch ms). */
  lastSyncedAt?: number;
};
...
export type SpriteCoreAgentEntry = {
  ...
  /** Optional link to the upstream pixellab.ai character driving this agent. */
  pixellab?: SpriteCorePixellabLink;
};
...
/**
 * PixelLab.ai bridge config. The dashboard uses this to drive
 * `create-character`, `animate-character`, and `export` flows server-side.
 * Without it, the dashboard's pixellab panels show a "not configured" state.
 */
export type SpriteCorePixellabBridgeConfig = {
  /** PixelLab API key. Env-backed `SecretRef` recommended. */
  apiKey?: SecretInput;
  /** Optional override for the API base; defaults to https://api.pixellab.ai/v2. */
  apiBase?: string;
};
...
export type SpriteCoreConfig = {
  ...
  pixellab?: SpriteCorePixellabBridgeConfig;
