...
export function validateAgentEntry(input: unknown): ValidationResult<SpriteCoreAgentEntry> {
  ...
  if (obj["pixellab"] !== undefined) {
    const link = validatePixellabLink(obj["pixellab"], "agent.pixellab", errors);
    if (link) {
      out.pixellab = link;
    }
  }
...
export function validatePixellabLink(
  input: unknown,
  path: string,
  errors: string[],
): SpriteCorePixellabLink | undefined {
  const obj = asObject(input, path, errors);
  if (!obj) {
    return undefined;
  }
  const characterId = asString(obj["characterId"], `${path}.characterId`, errors, {
    required: true,
  });
  if (characterId === undefined) {
    return undefined;
  }
  const out: SpriteCorePixellabLink = { characterId };
  if (obj["lastSyncedAt"] !== undefined) {
    if (typeof obj["lastSyncedAt"] !== "number" || !Number.isFinite(obj["lastSyncedAt"])) {
      errors.push(`${path}.lastSyncedAt: expected finite number`);
    } else {
      out.lastSyncedAt = obj["lastSyncedAt"];
    }
  }
  return out;
}
