...
if [[ $SKIP_BUILD -eq 0 ]]; then
  ...
  (cd "$REPO_ROOT" && pnpm --filter @tyler-rng/sprite-core-ui build)
else
...
# 3. Atomic swap into <install-dir>/node_modules/@tyler-rng/sprite-core/
TARGET="$INSTALL_DIR/node_modules/@tyler-rng/sprite-core"
NEW_DIR="${TARGET}.new"
rm -rf "$NEW_DIR"
mkdir -p "$NEW_DIR"
...
mv "$NEW_DIR" "$TARGET"
log "installed @tyler-rng/sprite-core at $TARGET"
